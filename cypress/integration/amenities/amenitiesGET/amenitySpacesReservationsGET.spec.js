...
describe('Unit ID Amenities API Routes Reservations for Tenant', () => {
  const headers = {
  ...
  it('Should access unit ID Amenities Reservations', () => {
    const props = ['self', 'first', 'last', 'next', 'prev'];
    const unit_id = Cypress.env('UNIT_ID_2');
    cy.request({
      headers,
      url: `${UNIT_API_ROUTES.UNITS}/${unit_id}/${AMENITIES_SPACES.AMENITY_SPACES}/${RESERVATIONS.RESERVATIONS}`,
      method: 'GET',
    }).then((res) => {
...
describe('Unit ID Amenities API Routes Reservations for an unauthorized Tenant', () => {
    const headers = {
      'Content-type': 'application/vnd.api+json',
      Accept: 'application/vnd.api+json',
    };
    ...
    it('Should not access unit ID Amenities Reservations', () => {
      const unit_id = Cypress.env('UNIT_ID_2');
      cy.request({
        headers,
        url: `${UNIT_API_ROUTES.UNITS}/${unit_id}/${AMENITIES_SPACES.AMENITY_SPACES}/${RESERVATIONS.RESERVATIONS}`,
